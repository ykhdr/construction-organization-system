...
func NewServer(listenAddr string, db *sql.DB) *Server {
	...
	constructionProjectService := service.NewConstructionProjectService(constructionProjectRepository)
...
func (s *Server) InitializeRoutes() {
	...
	api.HandleFunc("/building_site/{site-id:[0-9]+}/engineers", s.buildingSiteHandlers.GetEngineers).Methods("GET")
	...
	api.HandleFunc("/construction_management/{management-id:[0-9]+}/engineers", s.constructionManagementHandlers.GetEngineers).Methods("GET")
